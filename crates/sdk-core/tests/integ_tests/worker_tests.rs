...
use temporalio_common::{
    ...
    protos::{
        ...
        temporal::api::{
            ...
            workflowservice::v1::{GetWorkflowExecutionHistoryResponse, PollActivityTaskQueueResponse},
        },
    },
...
use temporalio_sdk_core::{
    ActivitySlotKind, ActivityTaskCompletionSuccess, CoreRuntime, LocalActivitySlotKind, PollError,
    PollerBehavior, RuntimeOptions,
...
impl MemoUpsertWarnWf {
    ...
    async fn run(ctx: &mut WorkflowContext<Self>) -> WorkflowResult<()> {
        ctx.upsert_memo([("key".to_string(), Payload {
            data: vec![0u8; 100],
            ..Default::default()
        })]);
        Ok(())
...
impl CombinedMemoWarnWf {
    ...
    async fn run(ctx: &mut WorkflowContext<Self>) -> WorkflowResult<()> {
        ...
        ctx.upsert_memo([
            ("k1".to_string(), Payload { data: vec![0u8; 60], ..Default::default() }),
            ("k2".to_string(), Payload { data: vec![0u8; 60], ..Default::default() }),
        ]);
        Ok(())
    }
}
...
impl SmallPartsActivities {
    #[activity]
    async fn two_part(_ctx: ActivityContext, _a: Vec<u8>, _b: Vec<u8>) -> Result<(), ActivityError> {
        Ok(())
    }
}
...
async fn payload_size_warn_workflow_start_input() {
    ...
    assert!(
        captured.lock().unwrap().iter().any(|m| m.contains("TMPRL1103")),
