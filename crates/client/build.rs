...
/// RPC methods whose request types carry payloads but should be forwarded without a
/// size check. Either checked upstream or exempt from payload limits.
const EXCLUDED_FROM_CHECK: &[&str] = &[
    "RespondWorkflowTaskCompleted",  // already checked in worker/client.rs
    "RespondActivityTaskCompleted",  // already checked in worker/client.rs
    "RespondWorkflowTaskFailed",     // Failure protos not subject to payload limits
    "RespondActivityTaskFailed",     // Failure protos not subject to payload limits
    "RespondActivityTaskFailedById", // Failure protos not subject to payload limits
];
...
fn generate_payload_checking_impl(
    ...
    for file in &descriptor_set.file {
        for service in &file.service {
            ...
            for method in &service.method {
                ...
                let excluded = EXCLUDED_FROM_CHECK.iter().any(|&e| e == rpc_name);
                if payload_containing.contains(input) && !excluded {
    ...
    output.push_str(");\n\ncheck_and_forward_workflow_methods!(\n");
