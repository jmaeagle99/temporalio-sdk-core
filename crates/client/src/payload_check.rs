//! Client-side payload size checking for WorkflowService calls.
...
use temporalio_common::{
    payload_visitor::{
        AsyncPayloadVisitor, MemoField, MemoObserver, MemoVisitable, PayloadField,
...
/// A [`WorkflowService`] wrapper that checks payload and memo sizes on outbound calls.
///
/// When `payload_error`/`memo_error` Arcs are set (worker path), calls that exceed the error
/// limit return an error and — for workflow/activity task completions — automatically resubmit
/// the appropriate failure to the server. When the Arcs are absent (client path), only the
/// warning threshold applies.
#[derive(Clone)]
pub(crate) struct PayloadCheckingWorkflowService {
    ...
    /// Server-provided payload error limit for the worker path (`None` on plain-client path).
    pub(crate) payload_error: Option<Arc<AtomicU64>>,
    /// Server-provided memo error limit for the worker path (`None` on plain-client path).
    pub(crate) memo_error: Option<Arc<AtomicU64>>,
}

static PAYLOADS_TOO_LARGE_SIZE_KEY: &str = "payloads-too-large-size";
static PAYLOADS_TOO_LARGE_LIMIT_KEY: &str = "payloads-too-large-limit";
static PAYLOADS_TOO_LARGE_KIND_KEY: &str = "payloads-too-large-kind";

/// Decodes a [`LimitExceeded`] from metadata previously stamped by
/// [`PayloadCheckingWorkflowService`]. Returns `None` if the marker key is absent.
#[doc(hidden)]
pub fn limit_exceeded_from_status(status: &tonic::Status) -> Option<LimitExceeded> {
    if !status
        .metadata()
        .contains_key(crate::PAYLOADS_TOO_LARGE_KEY)
    {
        return None;
    }
    let size = status
        .metadata()
        .get(PAYLOADS_TOO_LARGE_SIZE_KEY)
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse().ok())
        .unwrap_or(0u64);
    let limit = status
        .metadata()
        .get(PAYLOADS_TOO_LARGE_LIMIT_KEY)
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse().ok())
        .unwrap_or(0u64);
    let kind = match status
        .metadata()
        .get(PAYLOADS_TOO_LARGE_KIND_KEY)
        .and_then(|v| v.to_str().ok())
    {
        Some("memo") => "memo",
        _ => "payloads",
...
impl PayloadCheckingWorkflowService {
    fn load_error_limits(&self) -> (Option<u64>, Option<u64>) {
        let payload = self.payload_error.as_ref().and_then(|a| {
            let v = a.load(Ordering::Relaxed);
            (v > 0).then_some(v)
        });
        let memo = self.memo_error.as_ref().and_then(|a| {
            let v = a.load(Ordering::Relaxed);
            (v > 0).then_some(v)
        });
        (payload, memo)
...
/// Generates [`WorkflowService`] impls that check warn limits only (error limit always `None`).
/// Used for failure-proto RPCs whose payloads are failure details, not user results.
macro_rules! check_warn_and_forward_methods {
    ($(($method:ident, $req:ty, $resp:ty));* $(;)?) => {
        $(
            fn $method(
                &mut self,
...
macro_rules! check_error_and_forward_methods {
    ($(($method:ident, $req:ty, $resp:ty));* $(;)?) => {
        $(
            fn $method(
                &mut self,
                ...
                let pw = self.payload_warn;
                let mw = self.memo_warn;
                let (pe, me) = self.load_error_limits();
                let mut inner = self.inner.clone();
                Box::pin(async move {
                    let result = check_payload_limits(
                        request.get_mut(),
                    ...
                    if let Some(exceeded) = result.error_exceeded {
                        let mut status = Status::invalid_argument(exceeded.message());
                        {
                            let meta = status.metadata_mut();
                            meta.insert(crate::PAYLOADS_TOO_LARGE_KEY,
                                tonic::metadata::MetadataValue::from(0));
                            if let Ok(v) = tonic::metadata::MetadataValue::try_from(
                                exceeded.size.to_string().as_str()) {
                                meta.insert(PAYLOADS_TOO_LARGE_SIZE_KEY, v);
                            }
                            if let Ok(v) = tonic::metadata::MetadataValue::try_from(
                                exceeded.limit.to_string().as_str()) {
                                meta.insert(PAYLOADS_TOO_LARGE_LIMIT_KEY, v);
                            }
                            meta.insert(PAYLOADS_TOO_LARGE_KIND_KEY,
                                tonic::metadata::MetadataValue::from_static(exceeded.kind));
                        }
                        return Err(status);
                    }
                    inner.$method(request).await
...
mod tests {
    ...
    fn limit_exceeded_message_error() {
        ...
        let msg = e.message();
        assert!(
            msg.contains("TMPRL1103"),
            "message should contain TMPRL1103: {msg}"
        );
        assert!(
            msg.contains("error"),
            "message should contain 'error': {msg}"
        );
    }
    ...
    fn limit_exceeded_message_warn() {
        ...
        assert!(
            ...
            "message should contain 'warning': {msg}"
        );
    }

    /// Verify that every entry in `WARN_ONLY_FROM_ERROR_CHECK` in `build.rs` refers to a real
    /// payload-carrying RPC. A stale entry would be silently dead — this catches that.
    #[test]
    fn warn_only_entries_are_valid_payload_rpcs() {
        let build_src = include_str!("../build.rs");
        // Extract the slice literal body of WARN_ONLY_FROM_ERROR_CHECK.
        let block = build_src
            .split("const WARN_ONLY_FROM_ERROR_CHECK")
            .nth(1)
            .and_then(|s| s.split("];").next())
            .expect("WARN_ONLY_FROM_ERROR_CHECK constant not found in build.rs");
        ...
        assert!(
            !entries.is_empty(),
            "Could not parse any entries from WARN_ONLY_FROM_ERROR_CHECK in build.rs — check the \
             format"
        );
        ...
        for &name in &entries {
            assert!(
                ...
                "WARN_ONLY_FROM_ERROR_CHECK entry '{name}' in build.rs is not a known \
                 payload-carrying RPC. Either the RPC was renamed/removed, or it never carried \
                 payloads. Remove it from the list."
            );
        }
    }
}
